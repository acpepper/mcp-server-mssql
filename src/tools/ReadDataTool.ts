...
export class ReadDataTool implements Tool {
  ...
  /**
   * Adds NOLOCK hints to SELECT queries when in read-only mode
   * @param query The original SQL query
   * @returns Modified query with NOLOCK hints
   */
  private addNolockHints(query: string): string {
    if (!this.isReadOnly) {
      return query;
    }

    // Simple regex to add NOLOCK hints to table references in SELECT queries
    // This handles most common patterns like "FROM table", "FROM schema.table", "JOIN table", etc.
    let modifiedQuery = query;

    // Add NOLOCK to FROM clauses
    ...
    modifiedQuery = modifiedQuery.replace(
      /(JOIN|LEFT JOIN|RIGHT JOIN|INNER JOIN|OUTER JOIN)\s+(\[?[a-zA-Z_][a-zA-Z0-9_]*\]?\.?\[?[a-zA-Z_][a-zA-Z0-9_]*\]?)/gi,
      '$1 $2 WITH (NOLOCK)'
    );
  ...
  async run(params: any) {
    try {
      ...
      const finalQuery = this.addNolockHints(query);

      // Log the query for audit purposes (in production, consider more secure logging)
      console.log(`Executing validated SELECT query: ${finalQuery.substring(0, 200)}${finalQuery.length > 200 ? '...' : ''}`);
